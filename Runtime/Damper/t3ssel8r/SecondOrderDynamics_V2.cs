...
{
    // Video 13:53
    public class SecondOrderDynamics_V2
    {
        Vector3 xp; // previous input
        Vector3 y, yd; // state variables
        float k1, k2, k3; // dynamics constrants

        public SecondOrderDynamics_V2(float f, float z, float r, Vector3 x0)
        {
            // compute constants
            k1 = z / (Mathf.PI * f);
        ...
        public Vector3 Update(float T, Vector3 x, Vector3 xd)
        {
            ...
            float k2_stable = Mathf.Max(k2, T * T / 2f + T * k1 / 2f, T * k1); // clamp k2 to guarantee stability without jitter
